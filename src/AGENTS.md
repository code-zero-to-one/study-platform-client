...
## Purpose
All application source code for the ZERO-ONE study platform. Organized into 14 modules following a hybrid architecture: feature-based modules (`features/`) coexist with a traditional domain-separated structure (`components/`, `hooks/`, `types/`). The two patterns must not be mixed within a single PR.
...
- **Features vs Components**: New feature code goes in `features/<domain>/`. Shared UI goes in `components/common/`. Don't mix structures in a single PR.
