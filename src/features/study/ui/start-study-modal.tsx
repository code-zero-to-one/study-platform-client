   ...
   return (
      <div className="flex flex-col gap-150">
         ...
         <div className="bg-background-alternative rounded-75 px-200 py-300">
            <ul className="list-disc list-outside pl-6 font-designer-15r text-text-subtle mx-250">
               {items.map((item: string, idx: number) => (
                  <li key={idx} className='mb-100 last:mb-0'>{item}</li>
               ))}
            </ul>
         </div>
      </div>
...
export function LabeledField({
   ...
   return (
      <div className={cn('flex flex-col', className)}>
         <label className="font-designer-16b text-text-default inline-block mb-100">
            {label}
            {required && (
...
export default function StartStudyModal() {
   ...
   const isFormValid = introduce.trim() !== ''
      && studyPlan.trim() !== ''
      && phoneNumber.trim() !== ''
