...
import type {
  ...
  AdminLessonNotionSyncStatusResponse,
...
import { normalizeAdminRetrospectivePurpose } from '@/features/admin/course-management/model/admin-course-presentation';
...
const normalizeAdminLessonDetail = (
  ...
  retrospectivePurpose: normalizeAdminRetrospectivePurpose(
    response.retrospectivePurpose,
  ),
...
export const linkAdminLessonNotionPage = async ({
  lessonId,
  request,
}: {
  lessonId: number;
  request: AdminLessonNotionLinkRequest;
}): Promise<AdminLessonNotionSyncStatusResponse> => {
  const response = await axiosInstanceV5.post<
    ApiBaseResponse<AdminLessonNotionSyncStatusResponse>
  >(`admin/lessons/${lessonId}/notion-link`, request);

  return unwrap(response);
};

export const unlinkAdminLessonNotionPage = async (
  lessonId: number,
): Promise<AdminLessonNotionSyncStatusResponse> => {
  const response = await axiosInstanceV5.delete<
    ApiBaseResponse<AdminLessonNotionSyncStatusResponse>
  >(`admin/lessons/${lessonId}/notion-link`);

  return unwrap(response);
};

export const getAdminLessonNotionSyncStatus = async (
  lessonId: number,
): Promise<AdminLessonNotionSyncStatusResponse> => {
  const response = await axiosInstanceV5.get<
    ApiBaseResponse<AdminLessonNotionSyncStatusResponse>
  >(`admin/lessons/${lessonId}/notion-sync/status`);

  return unwrap(response);
};

export const previewAdminLessonNotionSync = async (
  lessonId: number,
): Promise<AdminLessonNotionSyncPreviewResponse> => {
  const response = await axiosInstanceV5.get<
    ApiBaseResponse<AdminLessonNotionSyncPreviewResponse>
  >(`admin/lessons/${lessonId}/notion-sync/preview`);

  return unwrap(response);
};

export const syncAdminLessonNotionContent = async (
  lessonId: number,
): Promise<AdminLessonNotionSyncApplyResponse> => {
  const response = await axiosInstanceV5.post<
    ApiBaseResponse<AdminLessonNotionSyncApplyResponse>
  >(`admin/lessons/${lessonId}/notion-sync`);

  const body = unwrap(response);

  return {
    ...body,
    lesson: normalizeAdminLessonDetail(body.lesson),
  };
};

export const importAdminLessonNotionPage = async ({
  lessonId,
  request,
}: {
  lessonId: number;
  ...
  await linkAdminLessonNotionPage({ lessonId, request });

  return syncAdminLessonNotionContent(lessonId);
};
