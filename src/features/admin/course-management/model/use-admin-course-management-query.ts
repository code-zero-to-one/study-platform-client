...
import {
  ...
  getAdminLessonNotionSyncStatus,
  getAdminLessonQnaDetail,
  getAdminLessonQnas,
  getAdminLessonRetrospectives,
  importAdminLessonContentZip,
  importAdminLessonNotionPage,
  linkAdminLessonNotionPage,
  previewAdminLessonNotionSync,
  reorderAdminLessons,
  syncAdminLessonNotionContent,
  unlinkAdminLessonNotionPage,
...
export const adminCourseManagementQueryKeys = {
  ...
  lessonNotionSyncStatus: (lessonId?: number) =>
    [
      ...adminCourseManagementQueryKeys.all,
      'lesson-notion-sync-status',
      lessonId,
    ] as const,
};
...
export const useAdminLessonNotionSyncStatusQuery = (lessonId?: number) =>
  useQuery({
    queryKey: adminCourseManagementQueryKeys.lessonNotionSyncStatus(lessonId),
    queryFn: () => getAdminLessonNotionSyncStatus(lessonId ?? 0),
    enabled: typeof lessonId === 'number',
    staleTime: 15_000,
  });
...
export const useLinkAdminLessonNotionPageMutation = () => {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    ...
    }) => linkAdminLessonNotionPage({ lessonId, request }),
    onSuccess: async (status) => {
      useToastStore
        .getState()
        .showToast('Notion 페이지를 연결했습니다.', 'success');
      queryClient.setQueryData(
        adminCourseManagementQueryKeys.lessonNotionSyncStatus(status.lessonId),
        status,
      );
      await queryClient.invalidateQueries({
        queryKey: adminCourseManagementQueryKeys.lessonNotionSyncStatus(
          status.lessonId,
        ),
      });
    },
...
export const useUnlinkAdminLessonNotionPageMutation = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: unlinkAdminLessonNotionPage,
    onSuccess: async (status) => {
      useToastStore
        .getState()
        .showToast('Notion 연결을 해제했습니다.', 'success');
      queryClient.setQueryData(
        adminCourseManagementQueryKeys.lessonNotionSyncStatus(status.lessonId),
        status,
      );
...
export const usePreviewAdminLessonNotionSyncMutation = () =>
  useMutation({
    mutationFn: previewAdminLessonNotionSync,
    onError: showMutationError,
  });

export const useSyncAdminLessonNotionContentMutation = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: syncAdminLessonNotionContent,
    onSuccess: async (response) => {
      useToastStore
        .getState()
        .showToast(
      ...
      queryClient.setQueryData(
        adminCourseManagementQueryKeys.lessonDetail(response.lessonId),
        response.lesson,
      );
      await queryClient.invalidateQueries({
        queryKey: adminCourseManagementQueryKeys.lessonDetail(
...
export const useImportAdminLessonNotionPageMutation = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: importAdminLessonNotionPage,
    onSuccess: async (response) => {
      useToastStore
        .getState()
        .showToast(
          response.changed
            ? '노션데이터를 가져와 본문을 저장했습니다.'
            : '노션데이터가 이미 최신 본문입니다.',
          'success',
        );
      queryClient.setQueryData(
