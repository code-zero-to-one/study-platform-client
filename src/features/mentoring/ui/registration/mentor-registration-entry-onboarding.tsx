...
export default function MentorRegistrationEntryOnboarding({
  ...
  const complete = () => {
    ...
    onComplete({ ...values, appealLine: values.appealLine.trim() });
  };

  return (
    <div className="mx-auto flex min-h-[90dvh] w-full max-w-[980px] items-center px-150 py-200 sm:px-400">
      {' '}
      <section className="rounded-200 border-border-subtle bg-background-default w-full overflow-hidden border">
        ...
        <header className="from-fill-brand-subtle-default/70 to-background-default border-border-subtle border-b bg-linear-to-r px-200 py-200 sm:px-300 sm:py-250">
          ...
          <div className="mb-125 flex items-center justify-between gap-100">
            ...
            <div className="rounded-500 bg-background-default inline-flex items-center gap-75 px-100 py-50">
              {' '}
              <Sparkles className="h-14 w-14 text-text-brand" />{' '}
            ...
            <button
              type="button"
              className="hover:text-text-default font-designer-13b text-text-subtle"
              onClick={onSkip}
            >
              {' '}
              건너뛰고 직접 입력{' '}
          ...
          <div className="mb-75 flex items-center gap-100">
            {' '}
            <MessageCircle className="h-20 w-20 text-text-brand" />{' '}
            <h1 className="font-designer-28b text-text-default">
              {' '}
              어떤 멘토로 활동하실 건가요?{' '}
            </h1>{' '}
          </div>{' '}
          <p className="mb-150 font-designer-14r text-text-subtle">
            {' '}
        ...
        <div className="px-200 py-250 sm:px-300 sm:py-300">
          {' '}
          <p className="mb-50 font-designer-12b text-text-brand">
            {' '}
            {step} / {STEP_ORDERS.length} 단계{' '}
          </p>{' '}
          <h2 className="mb-75 font-designer-22b text-text-default">
            {' '}
            {getStepTitle(step)}{' '}
          </h2>{' '}
          <p className="mb-200 font-designer-13r text-text-subtle">
            {' '}
            {getStepDescription(step)}{' '}
          </p>{' '}
          {step === 1 && (
            <div className="grid grid-cols-1 gap-100 sm:grid-cols-4">
              {' '}
              {jobGroupOptions.map((group) => (
                <button
                  ...
                  type="button"
                  className={cn(
                    'rounded-125 border px-125 py-125 text-left transition-colors font-designer-14b',
                    values.jobGroup === group.code
                      ? 'border-border-brand bg-fill-brand-subtle-default text-text-brand'
                      : 'border-border-subtle bg-background-default hover:border-border-brand text-text-default',
                  )}
                  onClick={() => {
                    setValues((prev) => ({
                      ...prev,
            ...
            <div className="grid grid-cols-1 gap-100 sm:grid-cols-2">
              ...
              {jobTitleOptions.map((option) => (
                <button
                  ...
                  type="button"
                  className={cn(
                    'rounded-125 border px-125 py-125 text-left transition-colors font-designer-14m',
                    values.jobTitle === option.code
                      ? 'border-border-brand bg-fill-brand-subtle-default text-text-brand'
                      : 'border-border-subtle bg-background-default hover:border-border-brand text-text-default',
                  )}
                  onClick={() => {
                    setValues((prev) => ({ ...prev, jobTitle: option.code }));
                    setStep(3);
                  }}
                >
                  {' '}
            ...
            <div>
              ...
              <BaseInput
                ...
                onChange={(event) => {
                  ...
                  setValues((prev) => ({ ...prev, appealLine: nextValue }));
                }}
                placeholder="한 줄로 강점을 알려주세요"
                maxLength={24}
              />{' '}
              <p className="mt-75 font-designer-12r text-text-subtle">
                {' '}
              ...
              <div className="mt-125 flex flex-wrap gap-75">
                ...
                {MENTOR_APPEAL_LINE_PRESETS.map((preset) => (
                  <ChipButton
                    ...
                    variant="preset"
                    onClick={() => {
                      setValues((prev) => ({ ...prev, appealLine: preset }));
                    }}
                  >
                    {' '}
                    {preset}{' '}
