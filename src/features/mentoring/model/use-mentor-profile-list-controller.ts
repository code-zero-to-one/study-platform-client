...
import {
  getLocalMentorCareerOptions,
  getLocalMentorDirectoryPage,
  getMentorDirectoryErrorMessage,
...
export const useMentorProfileListController = ({
  ...
  const localFallbackPage = useMemo(() => {
  ...
  const fallbackEnabled = isError && shouldUseLocalMentorFallback(error);
  const localCareerOptions = useMemo(() => {
    return getLocalMentorCareerOptions(createdMentors);
  }, [createdMentors]);
  const careerOptions = useMemo(() => {
    if (mentorRegistrationOptionsQuery.data?.careers.length) {
      return mentorRegistrationOptionsQuery.data.careers
  ...
  const effectiveData = fallbackEnabled ? localFallbackPage : data;
  const mentors = useMemo(() => effectiveData?.mentors ?? [], [effectiveData]);
  ...
  const errorMessage = useMemo(() => {
    return (
      getMentorDirectoryErrorMessage(error) ?? DEFAULT_DIRECTORY_ERROR_MESSAGE
    );
  }, [error]);
  ...
  const totalPages = Math.max(1, effectiveData?.totalPages ?? 1);
  ...
  const handleRetry = () => {
    if (fallbackEnabled) {
      showToast(
        '검증용 멘토 목록을 표시 중입니다. 실제 멘토 목록은 백엔드 연결 후 다시 확인해주세요.',
        'info',
      );
