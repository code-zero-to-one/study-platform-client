...
const toWeeklySlotsFromWeeklyObject = (source: WeeklyResponseDto | undefined) => {
  return {
...
const toWeeklySchedule = (source: ScheduleResponseDto | undefined) => {
  try {
    const weeklyFromRanges = toWeeklySlotsFromRangesObject(source?.weeklyRanges);
...
export const mapRegistrationOptionsContent = (
  ...
  return {
    maxCoreKeywordCount,
    jobGroups: toSortedByDisplayOrder(mapRegistrationJobGroups(jobGroupsSource)),
    jobTitles: toSortedByDisplayOrder(mapRegistrationJobTitles(jobTitlesSource)),
