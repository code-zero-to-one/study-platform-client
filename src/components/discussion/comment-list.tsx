...
import type { BalanceGameComment } from '@/types/balance-game';
import type { DiscussionComment } from '@/types/discussion';
import type { CommentFormData } from '@/types/schemas/zod-schema';
import type { VotingComment, VotingOption } from '@/types/voting';

type CommentAuthor =
  | DiscussionComment['author']
  | VotingComment['author']
  | BalanceGameComment['author'];
...
function getAuthorImageUrl(author: CommentAuthor): string | undefined {
      ...
      {comments.map((comment) => {
        ...
        if (votedOption && votingOptions) {
          const optionIndex = votingOptions.findIndex(
            (option) => option.label === votedOption,
          );
          if (optionIndex !== -1) {
            optionColor =
