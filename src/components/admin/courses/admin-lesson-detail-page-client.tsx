...
export default function AdminLessonDetailPageClient({
  ...
  const notionSyncStatusQuery = useAdminLessonNotionSyncStatusQuery(lessonId);
  ...
  const [lessonBodyViewMode, setLessonBodyViewMode] = useState<'edit' | 'view'>(
    'edit',
  );
    ...
    importLessonNotionPageMutation.mutate(
      ...
      },
      {
        onSuccess: (response) => {
          ...
          setHydratedLessonSnapshot(
            toAdminLessonHydrationSnapshot(response.lesson),
          );
        },
      },
    );
    ...
    <main className="flex flex-col gap-200 p-200">
      ...
      <section className="border-border-default bg-background-default rounded-150 border p-200">
        ...
        <div className="mt-150 flex flex-col gap-150">
          ...
          <div className="rounded-125 border border-border-brand bg-fill-brand-subtle-default px-150 py-125">
            ...
            <div className="font-designer-12r text-text-subtle mt-100 flex flex-wrap gap-125">
              <span>
                Notion 수정:{' '}
                {formatDateTime(
                  notionSyncStatusQuery.data?.notionLastEditedTime ?? null,
                )}
              </span>
              <span>
                마지막 동기화:{' '}
                {formatDateTime(
                  notionSyncStatusQuery.data?.lastSyncedAt ?? null,
                )}
              </span>
          ...
          <div className="flex items-center justify-end gap-50">
            <Button
              type="button"
              size="xsmall"
              color={lessonBodyViewMode === 'edit' ? 'primary' : 'outlined'}
              onClick={() => setLessonBodyViewMode('edit')}
            >
              편집
            </Button>
            <Button
              type="button"
              size="xsmall"
              color={lessonBodyViewMode === 'view' ? 'primary' : 'outlined'}
              onClick={() => setLessonBodyViewMode('view')}
            >
              수강생 화면
            </Button>
          </div>
          {lessonBodyViewMode === 'edit' ? (
            ...
            <AdminCourseField
              label="본문"
              helper="실제 수강생에게 보여지는 레슨 본문과 동일한 렌더러로 표시됩니다."
            >
              <div className="border-border-default rounded-100 min-h-260px border p-150">
                <MarkdownContentCore content={lessonForm.content} />
              </div>
            </AdminCourseField>
          )}
        </div>
            ...
            {lessonRetrospectivesQuery.data?.retrospectives.map((retro) => (
              ...
              >
                <div className="mb-75 flex flex-wrap items-center gap-75">
                  <Badge
                    color={
                      getAdminRetrospectivePurposeMeta(retro.purpose).color
                    }
                  >
