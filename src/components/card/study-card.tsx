...
export default function StudyCard({ study, href, onClick }: StudyCardProps) {
  ...
  return (
    <Link
      href={href}
      onClick={onClick}
      className={`hover:shadow-2 hover:border-border-brand rounded-150 cursor-pointer overflow-hidden border border-[#E5E7EB] transition-all ${isCompleted ? 'bg-gray-200 opacity-70' : 'bg-white'}`}
    >
      {/* 썸네일 영역 */}
