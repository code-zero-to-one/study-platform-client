...
export default function InquiryDetail({
  ...
  // 권한 체크 (강제 공개일 경우 무시)
  const hasPermission = isForceShown || canViewInquiry(
    inquiry,
    currentUserId,
    isMentor,
    isAdmin,
  ...
  if (!hasPermission) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] gap-300">
  ...
  return (
    <div className="flex flex-col gap-400">
      ...
      <div className="flex items-center gap-200">
        <button
          onClick={onBack}
          className="flex items-center gap-100 font-designer-14m text-text-subtle hover:text-text-default transition-colors"
        >
      ...
      {/* 문의 정보 카드 */}
      <div className="rounded-200 border border-border-default bg-white p-500">
        {/* 상단: 분류만 표시 */}
        <div className="flex items-center gap-200 mb-300">
          <span className="font-designer-13m text-text-subtle px-200 py-100 rounded-100 bg-background-neutral-subtle">
        ...
        {/* 작성자 정보 */}
        <div className="grid grid-cols-2 gap-x-400 gap-y-200 pb-300 mb-400 border-b border-border-default">
          <div className="flex items-center gap-200">
          ...
          <div className="flex items-center gap-200">
            <span className="font-designer-14m text-text-subtle">문의 상태</span>
            <InquiryStatusBadge status={inquiry.status} />
          </div>
        </div>

        {/* 본문 */}
        <div className="font-designer-16r text-text-default whitespace-pre-line mb-400">
          {inquiry.content}
        </div>

        {/* 첨부 이미지 */}
        {inquiry.images && inquiry.images.length > 0 && (
          <div className="flex flex-col gap-200 mt-400">
            <p className="font-designer-14b text-text-default">첨부 이미지</p>
            <div className="flex flex-wrap gap-200">
              {inquiry.images.map((image, index) => (
                <div
                  key={index}
                  className="relative w-[200px] h-[200px] rounded-100 overflow-hidden border border-border-default"
                >
                  <Image
                    src={image}
                    alt={`첨부 이미지 ${index + 1}`}
      ...
      {/* 답변 (있을 경우) */}
      {inquiry.answer && (
        <div className="rounded-200 border border-border-success bg-background-success-subtle p-500">
          <div className="flex items-center gap-200 mb-300">
      ...
      {canAnswer && (
        <div className="rounded-200 border border-border-default bg-white p-500">
          ...
          <textarea
            ...
            className="w-full rounded-100 border border-border-default px-300 py-200 font-designer-14m focus:border-border-brand focus:outline-none min-h-[200px] resize-none mb-300"
            maxLength={2000}
          />
