    ...
    <div className="flex flex-col gap-400">
      {sortedOptions.map((option, index) => {
        ...
        return (
          <div
            key={option.id}
            className={cn(
              'group relative overflow-hidden rounded-200 transition-all duration-300',
              isMyVote 
                ? 'border-2 border-fill-brand-default-default shadow-2' 
                : 'border border-border-subtle hover:border-border-brand hover:shadow-1',
            )}
          >
            {/* 배경 그라데이션 프로그레스 바 */}
            <div
            ...
            <div className="relative flex items-center justify-between px-500 py-400">
              ...
              <div className="flex items-center gap-300">
                ...
                <div className="flex flex-col gap-50">
                  ...
                  <div className="flex items-center gap-200">
                    ...
                    {/* 내가 투표한 항목 표시 */}
                    {isMyVote && (
                      <div className="flex items-center gap-50 rounded-100 bg-fill-brand-default-default px-200 py-50">
                        <Check className="h-3 w-3 text-text-inverse" />
                        <span className="font-designer-11b text-text-inverse">내 선택</span>
                      </div>
                    )}

                    {/* 1위 표시 */}
                    {isTopVote && (
                      <div className="flex items-center gap-50 rounded-100 bg-gradient-to-r from-yellow-400 to-orange-500 px-200 py-50">
                        <TrendingUp className="h-3 w-3 text-white" />
                        <span className="font-designer-11b text-white">1위</span>
                      </div>
                    )}
                  </div>
              ...
              {showPercentage && (
                <div className="flex flex-col items-end gap-50">
                  ...
                  <div className="h-1 w-16 rounded-full bg-border-subtle">
                    <div
                      className={cn('h-full rounded-full transition-all duration-1000', colors.primary)}
                      style={{ width: `${(option.percentage / topOption.percentage) * 100}%` }}
                    />
                  </div>
                </div>
              )}
            ...
            {/* 호버 효과용 빛나는 테두리 */}
            {!isMyVote && (
              <div className="absolute inset-0 rounded-200 opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                style={{
      ...
      <div className="mt-200 flex items-center justify-center gap-200 rounded-200 border border-border-subtle bg-background-alternative p-400">
        ...
        <div className="flex flex-col">
          <span className="font-designer-12r text-text-subtle">총 투표 참여</span>
          <span className="font-bold text-xl text-text-strong">
