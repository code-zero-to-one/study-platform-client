...
const getMarkdownEditorErrorMessage = (
  error: unknown,
...
export interface MarkdownEditorCoreProps {
  ...
  requestImageUploadTicket: ({
    ...
    fileType: string;
    fileSize: number;
  }) => Promise<MarkdownImageUploadTicket>;
  uploadImageFile: ({
    file,
    uploadUrl,
  }: {
...
function MarkdownEditorCore({
  ...
  const imageAcceptValue = useMemo(() => {
    return allowedImageExtensions.map((extension) => `.${extension}`).join(',');
  }, [allowedImageExtensions]);
  const maxImageFileSizeLabel = useMemo(() => {
    return formatFileSizeLimit(maxImageFileSize);
  }, [maxImageFileSize]);

  const isAllowedImageExtension = useCallback(
    (extension: string) => {
  ...
  const validateImageFile = useCallback(
    (file: File): string | undefined => {
      ...
      if (
        ...
        return `${file.name}: 허용되지 않은 확장자입니다. (${allowedExtensionsLabel})`;
      }

      if (file.size > maxImageFileSize) {
        return `${file.name}: ${maxImageFileSizeLabel}를 초과합니다.`;
      }
    ...
    <div className="rounded-125 border-border-subtle bg-background-default border">
      ...
      {isImageActive && (
        <div className="border-border-subtle flex items-center gap-100 border-b px-150 py-100">
          ...
          <input
            ...
            className="accent-background-brand-default w-rich-text-slider"
          />
      ...
      <EditorContent
        ...
        className={cn(
          ...
          'min-h-rich-text-editor w-full px-150 py-125',
        )}
      />
...
export default memo(MarkdownEditorCore);
