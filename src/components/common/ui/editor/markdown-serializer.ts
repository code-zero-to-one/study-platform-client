...
// --- 표(WYSIWYG table 노드 → GFM) --------------------------------------------
// main 에디터는 실제 TipTap table 노드(table > tableRow > tableHeader|tableCell)를
// 쓴다. 셀은 마크다운 소스를 리터럴 텍스트로 보유(renderMarkdownTablesInHtml가
// textContent로 채움)하므로, textContent + 파이프 escape로 GFM 표를 복원한다.
const serializeTableCellText = (cell: ProseMirrorNode): string =>
  cell.textContent.replace(/\r?\n/g, ' ').replace(/\|/g, '\\|').trim();

const serializeTableRow = (row: ProseMirrorNode): string => {
  const cells: string[] = [];
  row.forEach((cell) => {
    cells.push(serializeTableCellText(cell));
  });
  return `| ${cells.join(' | ')} |`;
};
...
const serializeTable = (
  ...
  node.forEach((row, _offset, rowIndex) => {
    ...
    state.write(serializeTableRow(row));
    state.ensureNewLine();
...
export const lessonMarkdownSerializer = new MarkdownSerializer(
  {
    ...
    // table 노드가 행/셀을 직접 직렬화하므로 아래는 호출되지 않는 안전망.
    tableRow() {},
    tableHeader() {},
    tableCell() {},
