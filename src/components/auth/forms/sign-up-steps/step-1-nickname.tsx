...
export function NicknameStep({ data, updateData, onNext }: NicknameStepProps) {
  ...
  // onNext는 비반응형 — useEffectEvent로 분리해 deps에서 제외.
  const advanceToNext = useEffectEvent(() => onNext());
  ...
  }, [isValidName, checked, isNicknameAvailable, isCheckingNickname]);
        ...
        <div className="flex w-full max-w-[320px] flex-col gap-200">
          ...
          <div className="flex items-center justify-center gap-100">
            ...
            >
              <div
                className={cn(
                  'rounded-50 flex size-[20px] items-center justify-center border transition-all',
                  checked
                    ? 'bg-fill-brand-default-default border-fill-brand-default-default'
                    : 'border-border-default bg-background-default',
                )}
              >
                {checked && <Check className="text-text-inverse size-175" />}
              </div>
