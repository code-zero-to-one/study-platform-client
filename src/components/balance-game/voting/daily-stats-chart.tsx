...
export default function DailyStatsChart({
  ...
  return (
    <div className="rounded-200 border-border-subtle bg-background-default border p-500">
      <h3 className="font-designer-16b text-text-strong mb-400">
        일별 투표 추이
      </h3>
      ...
      {myVote && myOptionTrend !== null && (
        <div
          className={cn(
            'rounded-100 mb-400 flex items-center gap-200 p-300',
        ...
        >
          ...
          <div>
            <p className="font-designer-13b text-text-strong">
              ...
              {myOptionTrend === 0 && (
                <span className="text-gray-600">변화 없음</span>
              )}
            </p>
      ...
      <div className="mb-300">
        <div className="relative" style={{ height: '200px' }}>
          {/* Y축 레이블 */}
          <div className="font-designer-11r text-text-subtlest absolute top-0 left-0 flex h-full flex-col justify-between pr-200 text-right">
          ...
          {/* 그리드 라인 */}
          <div className="absolute top-0 right-0 left-[40px] h-full">
            {[0, 25, 50, 75, 100].map((_, index) => (
              <div
                key={index}
                className="border-border-subtlest absolute w-full border-t"
                style={{ top: `${index * 25}%` }}
              />
            ))}
          </div>
          ...
          {/* 라인 차트 */}
          <svg
            className="absolute top-0 right-0 left-[40px] h-full w-[calc(100%-40px)]"
        ...
        {/* X축 레이블 */}
        <div className="border-border-subtle ml-[40px] flex justify-between border-t pt-100">
        ...
        {options.map((option, index) => {
          ...
          return (
            <div
              key={option.id}
              className={cn(
                'rounded-100 flex items-center gap-100 px-200 py-100',
