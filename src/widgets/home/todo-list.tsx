...
export default function TodoList({ statusList }: TodoListProps) {
   ...
   return (
      <section className="p-200 rounded-200 border border-border-subtle bg-background-default flex flex-col gap-150">
               ...
               return (
                  <li key={item.key} className="flex flex-col">
                     <div className='flex flex-row gap-75 items-center'>
                        ...
                        <span className={`font-designer-15m ${done ? "text-text-default" : "text-text-disabled"}`}>
                           {item.label}
                        </span>
                     </div>
                     {!isLast && (
                        <div className="w-[2px] h-[15px] ml-[13px] bg-border-subtle" />
                     )}
                  </li>
               );
            })}
