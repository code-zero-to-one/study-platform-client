...
모든 그룹스터디 목록을 조회합니다. 다양한 조건으로 필터링이 가능합니다. `my-participation`를 사용하면 현재 로그인한 사용자가 참가 중인 스터디만 조회합니다. `review-available`을 사용하면 현재 로그인한 사용자가 리뷰를 작성할 수 있는 스터디만 조회합니다. 이때 Pagination은 무시됩니다.  **[주의]** - 비회원 접근 가능 - 로그인 없이 조회할 수 있습니다. - `my-participation`가 제공될 시 로그인이 필요합니다. - `review-available`가 제공될 시 로그인이 필요합니다. 
...
let classification: 'GROUP_STUDY' | 'PREMIUM_STUDY' | 'MENTOR_STUDY'; //스터디 분류 (GROUP_STUDY / MENTOR_STUDY) (default to undefined)
let page: number; //페이지 번호 (1부터 시작) (optional) (default to 1)
let pageSize: number; //페이지 크기 (optional) (default to 20)
let myParticipation: 'PARTICIPANT' | 'LEADER'; //LEADER일 경우 현재 로그인한 사용자가 리더인 스터디만 조회합니다. PARTICIPANT일 경우 현재 로그인한 사용자가 리더가 아닌 참가자인 스터디만 조회합니다. (optional) (default to undefined)
...
let reviewAvailable: boolean; //리뷰 필터링 여부 (optional) (default to undefined)
...
|Name | Type | Description  | Notes|
|------------- | ------------- | ------------- | -------------|
| **classification** | [**&#39;GROUP_STUDY&#39; | &#39;PREMIUM_STUDY&#39; | &#39;MENTOR_STUDY&#39;**]**Array<&#39;GROUP_STUDY&#39; &#124; &#39;PREMIUM_STUDY&#39; &#124; &#39;MENTOR_STUDY&#39;>** | 스터디 분류 (GROUP_STUDY / MENTOR_STUDY) | defaults to undefined|
| **page** | [**number**] | 페이지 번호 (1부터 시작) | (optional) defaults to 1|
| **pageSize** | [**number**] | 페이지 크기 | (optional) defaults to 20|
| **myParticipation** | [**&#39;PARTICIPANT&#39; | &#39;LEADER&#39;**]**Array<&#39;PARTICIPANT&#39; &#124; &#39;LEADER&#39;>** | LEADER일 경우 현재 로그인한 사용자가 리더인 스터디만 조회합니다. PARTICIPANT일 경우 현재 로그인한 사용자가 리더가 아닌 참가자인 스터디만 조회합니다. | (optional) defaults to undefined|
...
| **reviewAvailable** | [**boolean**] | 리뷰 필터링 여부 | (optional) defaults to undefined|
