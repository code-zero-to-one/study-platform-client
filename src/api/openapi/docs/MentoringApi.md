...
멘토 등록 화면에서 사용하는 직군/직무/경력/핵심키워드 옵션을 조회합니다. 사용자 생성 핵심키워드는 이 응답에 포함되지 않습니다.
...
로그인한 사용자의 멘토 설정을 신규/수정 저장합니다. coreKeywords는 PREDEFINED(code) 또는 CUSTOM(label) 입력을 허용하며, CUSTOM 키워드는 저장되지만 등록 옵션 목록에는 자동 포함되지 않습니다.
