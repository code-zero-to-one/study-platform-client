...
# **createSettlement**
> createSettlement(studySettlementCreateRequest)
...
let status: string; //정산 상태 필터 (미전달 시 전체 조회) (optional) (default to undefined)
...
| **status** | [**string**] | 정산 상태 필터 (미전달 시 전체 조회) | (optional) defaults to undefined|
