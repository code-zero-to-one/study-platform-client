...
import { FreeCompletionSurvey } from './_components/free-completion-survey';
...
export default function CourseCompletePage() {
  ...
  const { data: freeEnrollment } = useGetMyCourseFreeEnrollment(courseId);
  const isFreeEnrolled = freeEnrollment?.isFreeEnrolled ?? false;
  const submitNextPlan = useSubmitNextPlan();

  useEffect(() => {
    // 무료 수료자에겐 설문 화면을 보여주므로 컨페티는 유료 경로 확정 후에만.
    if (freeEnrollment === undefined || isFreeEnrolled) return;
  ...
  if (isFreeEnrolled) {
    return <FreeCompletionSurvey courseId={courseId} recap={recap} />;
  }
