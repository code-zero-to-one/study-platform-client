...
// 한 주기 = A + B. translateX(-50%) 루프 위해 주기를 2회 복제 (seamless).
const MARQUEE_LOOP = [
  ...MARQUEE_ROW_A,
  ...MARQUEE_ROW_B,
  ...MARQUEE_ROW_A,
...
// Code/Community 카드 공통 표면 (rose radial gradient + 반투명). 위치·회전만 카드별로 다름.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <button
        ...
        onClick={onClose}
      />
      <div className="relative z-10 w-7000 overflow-hidden rounded-200 bg-background-default">
        <button
          type="button"
          onClick={onClose}
          ...
          <div className="mb-300 space-y-75">
            <label className="flex cursor-pointer items-start gap-125">
              <input
                ...
                className="mt-25 size-300 shrink-0 rounded-50 accent-rose-500"
              />
...
// 배너 장식 일러스트 (데스크톱 전용, 정적 자산). 본문 분리용 추출.
function BannerDecorations() {
  return (
    <div className="pointer-events-none absolute inset-0 hidden overflow-hidden lg:block">
      ...
      {/* Code card */}
      <div
        className="absolute flex size-[176px] flex-col items-center justify-center gap-75 rounded-250 border border-rose-300"
        style={{
          left: '40.17%',
          top: 71,
          ...CARD_SURFACE_STYLE,
      ...
      {/* Community card */}
      <div
        className="absolute size-[176px] rounded-250 border border-rose-300"
        style={{
          left: '48.65%',
          top: 124,
          ...CARD_SURFACE_STYLE,
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        {/* Hero heading */}
        <div className="relative z-10 flex min-h-[var(--min-h-hero-section)] flex-col items-center justify-center lg:min-h-7250 lg:justify-end lg:pb-550">
        ...
        <div className="flex h-800 items-center overflow-hidden bg-gray-1000">
          <div className="animate-marquee flex shrink-0 items-center gap-500 whitespace-nowrap px-500">
            {MARQUEE_LOOP.map((item, index) => (
              <span
                key={`${item}-${index}`}
                className="font-designer-24b text-gray-0"
              >
                {item}
              </span>
            ))}
