...
} from '@/hooks/queries/payment/payment-user-queries';
...
// 현재 시각 외부 스토어 — 마운트 후 1회만 Date.now() 캡처, 이후 안정값 유지.
let clientNowMs: number | undefined;
const nowListeners = new Set<() => void>();

function subscribeNow(listener: () => void) {
  nowListeners.add(listener);
  if (clientNowMs === undefined) {
...
function getNowServerSnapshot(): number | undefined {
  return undefined;
}
...
export default function PaymentManagement() {
  ...
  // 현재 시각(ms)을 외부 스토어로 구독 — SSR 스냅샷(undefined)과 클라이언트 스냅샷 분리로
  // 하이드레이션 불일치/깜빡임 없이 마운트 후 client 시각 확보. 렌더 중 new Date() 금지.
  ...
  return (
    <div className="flex flex-col gap-300">
      ...
      <div className="border-border-subtle flex border-b">
        <button
          type="button"
          className="font-designer-14m text-text-default border-border-brand border-b-2 pb-200 px-200"
        >
          스터디 결제
        </button>
        <button
      ...
      <DesktopTransactionTable
        ...
        expandedGroupStudyIds={expandedGroupStudyIds}
        nowMs={nowMs}
...
interface TransactionListProps {
  transactions: UserTransactionListResponse[] | undefined;
  expandedGroupStudyIds: Set<number>;
  nowMs: number | undefined;
  onToggle: (groupStudyId: number | undefined) => void;
}

function DesktopTransactionTable({
  transactions,
  expandedGroupStudyIds,
  nowMs,
  onToggle,
          ...
          {transactions && transactions.length > 0 ? (
            transactions.map((transaction) => {
              ...
              const beforeStarting =
                kstDate && nowMs !== undefined
                  ? kstDate.getTime() < nowMs
                  : false;
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {/* 맨 오른쪽: 토글 버튼 */}
                    <td className="p-300 text-center">
                      <button
                        type="button"
                        onClick={() => onToggle(transaction.groupStudyId)}
                      >
    ...
    <div className="flex flex-col gap-200 sm:hidden">
      {transactions && transactions.length > 0 ? (
        transactions.map((transaction) => {
          ...
          const kstDate = formatToKST(transaction.groupStudyStartDate);
          const beforeStarting =
            kstDate && nowMs !== undefined ? kstDate.getTime() < nowMs : false;
                ...
                <div className="flex flex-col gap-150 p-200">
                  ...
                  <div className="flex items-center justify-between gap-100">
                    <PaymentActionButtons
                      paymentId={transaction.paymentId}
                      groupStudyId={transaction.groupStudyId}
                      latestTransactionType={transaction.latestTransactionType}
                      paymentReceiptUrl={transaction.paymentReceiptUrl}
                      virtualAccountInfo={transaction.virtualAccountInfo}
                      layout="row"
                    />
  ...
  return (
    <tr className="border-b-border-default border-b">
      <td colSpan={4} className="px-250 pb-200">
        <div className="bg-background-alternative rounded-100 flex flex-col gap-50 pt-300 pr-200 pb-200 pl-250">
