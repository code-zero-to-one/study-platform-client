...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    // 빌드 시 origin 지연으로 hang 되지 않도록 클라이언트 타임아웃(30s) 부여
    // 한쪽만 타임아웃돼도 나머지 결과는 sitemap에 포함되도록 allSettled 사용
    const [normalResult, premiumResult] = await Promise.allSettled([
    ...
    const normalStudyItems: GroupStudyListItemDto[] =
      normalResult.status === 'fulfilled'
        ? (normalResult.value.data?.content?.content ?? [])
        : [];
    const premiumStudyItems: GroupStudyListItemDto[] =
      premiumResult.status === 'fulfilled'
        ? (premiumResult.value.data?.content?.content ?? [])
        : [];
