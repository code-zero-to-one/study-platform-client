...
const FEED_WRITE_PATH = '/class/vibe-intro/feed/write';
...
async function mockAndNavigate(page: Page): Promise<void> {
  await page.route(/\/courses\//, async (route) => {
    ...
    if (/\/courses\/\d+\/builder-feeds/.test(url) && method === 'POST') {
      await route.fulfill({ json: { content: { feedId: 1 } } });
  ...
  await Promise.all([
    ...
    page.goto(FEED_WRITE_PATH, { waitUntil: 'load' }),
  ]);
}

// Selects a lesson from the lesson dropdown
async function selectLesson(page: Page) {
  // Feed write page has two dropdowns (course + lesson); lesson is the second one
...
test.describe('피드 작성 폼 렌더링 @auth', () => {
...
test.describe('피드 제출 성공 @auth', () => {
  test('레슨 + 내용 입력 → POST /courses/1/builder-feeds + toast', async ({
    page,
  }) => {
    await mockAndNavigate(page);
    await selectLesson(page);
    ...
    const [response] = await Promise.all([
      page.waitForResponse(
        (r) =>
          /\/courses\/\d+\/builder-feeds/.test(r.url()) &&
    ...
    await expect(page.getByText('피드가 등록되었어요!')).toBeVisible({
      timeout: 5000,
    });
  });
});
